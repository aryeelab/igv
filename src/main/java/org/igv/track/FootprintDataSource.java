...
/**
 * Data source for footprint count matrix files (.counts.tsv.gz).
...
public class FootprintDataSource {
    ...
    private static final Logger log = LogManager.getLogger(FootprintDataSource.class);
    ...
    static class Record implements Comparable<Record> {
        ...
        Record(int position, int fragLen, int count) {
    ...
    public FootprintDataSource(ResourceLocator locator) throws IOException {
    ...
    /**
     * Build a FootprintMatrix for the given genomic range.
     *
     * @param chr          chromosome name
     * @param start        genomic start (inclusive)
     * @param end          genomic end (inclusive)
     * @param fragLenMin   minimum fragment length to include
     * @param fragLenMax   maximum fragment length to include
     ...
     * @return FootprintMatrix with processed data
     */
    public FootprintMatrix getMatrix(String chr, int start, int end,
        ...
        int numRows = fragLenMax - fragLenMin + 1;
        int numCols = range;
        ...
        if (chrRecords != null) {
            ...
            for (int i = idx; i < chrRecords.size(); i++) {
                ...
                int fragLen = rec.fragLen;
                if (fragLen > fragLenMax) fragLen = fragLenMax;
        ...
        if (applyScale && !scaleFactors.isEmpty()) {
            for (int row = 0; row < numRows; row++) {
                int fragLen = row + fragLenMin;
                Double factor = scaleFactors.get(fragLen);
        ...
        return new FootprintMatrix(matrix, chr, start, end, fragLenMin, fragLenMax);
    }
